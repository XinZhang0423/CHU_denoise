...
def rescale_imag(image_corrupt, scaling):
    ...
    if (scaling == 'standardization'):
        ...
        return image_corrupt / np.max(image_corrupt), 0, np.max(image_corrupt)
    else: # No scaling required
        return image_corrupt, 0, 0

def descale_imag(image, param_scale1, param_scale2, scaling='normalization'):
    """ Descaling of input """
    try:
        image_np = image.detach().numpy()
    except:
